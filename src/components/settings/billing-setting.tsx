...
const BillingSetting = async (props: Props) => {
  ...
  const planFeature = pricingCards.find((card)=>card.title.toUpperCase()==plan?.toUpperCase())?.features;
  return (
...

        {
          planFeature?.map((feature)=>(
            <div key={feature} className="flex gap-4">
              <CheckCircle2 className="text-muted-foreground"/> <p className="text-muted-foreground">{feature}</p>
            </div>
          ))
        }
        </div>
